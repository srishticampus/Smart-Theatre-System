...
const registerStaff = async (req, res) => {
  try {
    const { email, contact, password, fname, lname, role, doj, address } =
      req.body;
...
const editStaffById = async (req, res) => {
  ...
  try {
    ...
    return res
      .status(200)
      .json({ msg: "Updated successfully", data: updatedStaff });
  } catch (err) {
  ...
  Staff.findOneAndUpdate(
    ...
    .then((data) => {
      res
        .status(200)
        .json(
          data ? { msg: "Updated successfully" } : { msg: "Staff Not Found" }
        );
    })
    .catch((err) => {
      res.status(500).json({ msg: "Data not Updated", Error: err });
  ...
  Staff.findOne({ email })
    .then((user) => {
      ...
      if (!user.isActive) {
        return res
          .status(403)
          .json({ msg: "You are currently deactivated By Admin !!" });
      }
